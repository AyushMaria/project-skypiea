...
def takeoff(mav_connection, takeoff_altitude: float, autopilot_type, tgt_sys_id: int = 1, tgt_comp_id=1):
...
def change_mode(mav_connection, mode, autopilot_type, sub_mode='NONE'):
	
    mav_connection.wait_heartbeat()
    print("Heartbeat from system (system %u component %u)" %
          (mav_connection.target_system, mav_connection.target_component))	
    
    if autopilot_type == 'ardupilotmega':
...
if __name__ == "__main__":
	...
	for x in devices:
		if x.name == "STMicroelectronics GENERIC_F446RCTX HID in FS Mode" and x.phys == "usb-0000:00:14.0-1/input1":
		#if x.name == "AT Translated Set 2 keyboard":
			device = x
			break
	print(device)
	
	# determine autopilot software (PX4 or ArduPilot)
	autopilot_info = get_autopilot_info(mav_connection, tgt_sys_id) 
	autopilot_type = autopilot_info["autopilot"]
			...
			if device.fd in r:
				for event in device.read(): 
					if event.type==1 and event.code == 50:
						...
						result = takeoff(mav_connection, args.altitude, "ardupilot")
						...
						result =  change_mode(mav_connection, mode_apm_vtol[event.code], autopilot_type, "READY")
						print(f'Result of mode change command: {result}')
						
					elif event.type==1 and event.code in mode_apm_quad:
						result =  change_mode(mav_connection, mode_apm_quad[event.code], autopilot_type, "READY")
						print(f'Result of mode change command: {result}')
					...
					elif event.type==1 and event.code == 18:
		...
		while True:
			...
			if device.fd in r:
				for event in device.read(): 
					...
					elif event.type==1 and event.code in mode_px4_quad:
						mode = mode_px4_quad[event.code]
						if mode == "RETURN_TO LAUNCH":
							result = change_mode(mav_connection, "AUTO", autopilot_type, "RETURN_TO_LAUNCH")
						else:
							result =  change_mode(mav_connection, mode_px4_quad[event.code], autopilot_type, "READY")
						print(f'Result of mode change command: {result}')
